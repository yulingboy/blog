        ...
        <el-card class="box-card">
            ...
            <div>
                ...
                <el-input style="margin-top: 15px;" placeholder="请输入内容" v-model="cover">
                    <i slot="suffix" class="el-input__icon el-icon-upload" @click="clickUploadImage"></i>
                    <template slot="prepend">文章封面</template>
                </el-input>
                <input style="display: none" ref="uploadImage" type="file" name="uploadimage" accept="image/*" @change="setImage" />
                <el-row :gutter="10">
                    <!-- 文章标签  -->
                    <el-col :span="14">
                        <!-- 文章标签 -->
                    ...
                    <el-col :span="4">
                        <div class="grid-content bg-purple">
                            <el-select v-model="isPublishValue" placeholder="是否发布" style="margin-top: 15px;">
                                <el-option v-for="item in isPublish" :key="item.value" :label="item.label" :value="item.value"></el-option>
                            </el-select>
                        </div>
                    </el-col>
                    <!-- 提交 -->
            ...
            <div class="btn-list">
                ...
                <input type="file" id="files" ref="refFile" style="display: none" v-on:change="getdata" />
            </div>
            <!-- 输入区域 -->
...
export default {
    ...
    data() {
        return {
            ...
            title: '',
            // 默认封面
            cover: 'http://images.bianxiaofeng.com/ced846eab08578468670a87333e05b5f.jpg',
            // 文章描述
            description: '',
            options: [],
            // 是否发布
    ...
    methods: {
            ...
            if (this.$route.params.id) {
                ...
                await this.$api
                    ...
                    .then(res => {
                        ...
                        this.$store.state.markdown = res.data.content;
                    })
                    .catch(err => {
                        console.log(err);
                    });
        ...
        addArticle: async function() {
            ...
            console.log(addForm);
            if(this.$route.params.id){
              await this.$api.articleEdit(this.$route.params.id,addForm).then(res => {
                console.log(res);
                this.title = '';
                this.cover = '';
                this.description = '';
                ...
                this.dynamicTags = '';
                this.value = '';
                this.$store.state.markdown= null
                this.$message.success('修改成功！');
                this.$router.push('/article');
            });
            }else{
              await this.$api.articleAdd(addForm).then(res => {
                console.log(res);
                this.title = '';
                this.cover = '';
                this.description = '';
        ...
        uploadImage() {
            ...
            this.$api.upload(data).then(res => {
                ...
                this.cover = res.data.avatar_url;
                this.upImgDialogVisible = false;
            });
        },
        updateData(e) {
        ...
        getdata(e) {
            ...
            reader.onload = function() {
                ...
                _this.$store.state.markdown = this.result;
            };
        }
    }
};
